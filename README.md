...
配置和打包：
由于这里属于cs模式并且使用到gcc进行打包，代码里面下面的配置需要更改并打包：
1. 使用飞书代理的：lark_send_markdown.go文件，const larkproxy = "设置飞书代理"
2. 指定集群默认连接地址：def.go，const (
   StarRocksServer   = "生产SR连接地址"
   StarRocksServerQa = "测试SR连接地址"
   )
3. 这里使用了一个mysql配置表：chengken.starrocks_starload_cf，这里管理着版本的可用性。