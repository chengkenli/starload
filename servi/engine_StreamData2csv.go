...
// toCsv 慢查询落表
func toCsv(citem *util.ConnectData, dataFile, spacer, filterRatio, skipheader string) error {
	...
	response, err := client.SetRedirectPolicy(resty.RedirectPolicyFunc(func(req *http.Request, via []*http.Request) error {
		...
		SetHeaders(map[string]string{
			...
			"skip_header":      skipheader,                 /*用于指定跳过 CSV 文件最开头的几行数据*/
	...
	if result.Status == "Success" {
		...
		return errors.New(fmt.Sprintf("load failed %s %v", label, result.Message))
	}
}
